...
try:
    opts, args = getopt.getopt(
        sys.argv[1:], "i:d:", ["input_json_file=", "days_to_sync="]
    )
...
for camera in cameras_to_sync:
    ...
    try:
        for day in day_capture_directories:
            ...
            files_to_sync = client.list_from_directory(
                camera_directory_to_sync + "/captures"
            )

            for f in files_to_sync:
                local_directory = (
                    prisma_capture_diretory + "/" + camera_name + "/" + day[:6]
                )
                if not os.path.isdir(local_directory):
                    os.makedirs(local_directory)
                remote_file = (
                    camera_directory_to_sync + "/captures/" + str(f.decode())
                )
                local_file = local_directory + "/" + str(f.decode())
                if (
                    not os.path.isfile(local_file)
                    or not client.size_of_file(remote_file)
                    == os.stat(local_file).st_size
                ):
                    client.download_file(remote_file, local_file)

            events_folder_to_sync = client.list_from_directory(
                camera_directory_to_sync + "/events"
            )
            for event_folder in events_folder_to_sync:
                complete_remote_path_to_sync = (
                    camera_directory_to_sync + "/events"
                )
                files_to_sync = client.list_from_directory(
                    camera_directory_to_sync
                    + "/events/"
                    + str(event_folder.decode())
                )
                for f in files_to_sync:
                    remote_file = (
                        camera_directory_to_sync
