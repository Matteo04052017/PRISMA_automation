...
pro calibration, camera, target, process_image=process_image, process_day=process_day, process_month=process_month, config_file=config_file

compile_opt idl2

if ~isa(process_image) then process_image = 1
if ~isa(process_day)   then process_day   = 1
if ~isa(process_month) then process_month = 1
...
if process_image or process_day then begin
  ...
  for i = 0, n_elements(start)-1 do begin
    ...
    if finish[i] - start[i] gt 1 then begin
      ...
      if process_image then astrometry_image, par, days[i], captures
